...
1. **Stage.** Prefer naming the files you intend to commit explicitly — `git add <paths>` beats `git add -A` / `git add .`. Blind wildcards can sweep in secrets, build artifacts, and editor scratch files. If specific files are already staged, commit only those. Auto-staging all modified and new files is a *fallback* only when no explicit paths are obvious.
...
5. **Present the plan** to the user — files to be committed, subject, and body — and **ask for confirmation before running `git commit`**. On multi-commit splits, list each commit's files and subject.
...
Do not run `git commit` until you have presented the plan (files + subject + body) and the user has confirmed. This gate is mandatory — surface the proposed message and the exact files first, then wait for the go-ahead.
...
git commit \
  ...
  --message "- Added POST /auth/refresh accepting a refresh token
...
- **Confirm before committing.** Present the plan — files, subject, body — and wait for the user's go-ahead before running `git commit`.
...
- **Specific paths over wildcards.** `git add <paths>` beats `git add -A` / `git add .`. This avoids accidental staging of secrets, build output, and editor scratch files. Auto-staging everything is a fallback only when no explicit paths are obvious.
- **One logical change per commit.** If the staged diff touches multiple unrelated changes, split into multiple commits.
