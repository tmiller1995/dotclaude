...
If the structure outline is missing, or the path points at a research doc instead, abort — see When to Abort This Phase.

### Step 3: Verify Frozen Decisions

Quickly scan the design document. Are all open questions resolved? If it still has an unresolved `## Open Questions` section, abort — see When to Abort This Phase.
...
The output skill renders these via the **slice-card component** — see `skills/output-html/references/components.md` → 'Vertical-slice cards' (or `skills/output-markdown/components.md` for the MD variant). The required children are the heading, the goal, the files-touched list, and the checkpoint; Implementation steps, Tests, Automated verification, and Manual verification are the create-spec-specific additions. Do NOT invent new component classes — use the slice-card contract exactly as the output skills define it.
...
| `META_EXTRA` | `crispy:structure_doc`, `crispy:design_doc`, `crispy:research_doc`, `crispy:author`, `crispy:phase_count` |
| `SUMMARY_EXTRA` | rows for: Phases (accent), Structure doc (link), Design doc (link), Author |

**Author metadata.** The output skill already stamps the date into both the head metadata and the summary, so leave `crispy:date` and the `Date` row out of the brief — repeating them renders the field twice. The author is not in the template, so carry it explicitly: run `git config user.name` (Bash), put the result in `META_EXTRA` as `crispy:author`, and add a matching `Author` row to `SUMMARY_EXTRA`.
...
4. Suggest the next command — `Use the planner agent to decompose research/specs/YYYY-MM-DD-topic.<ext> into tasks` when the work should become a dependency-wired task list workers pick up one at a time, or `/implement` to run the phases directly for smaller features
